...
const ProjectCard = ({ project }: ProjectCardProps): JSX.Element => {
  const statusClass = statusColors[project.status] || "bg-gray-500/20 text-gray-400 border-gray-500/30";
  ...
  return (
    <div className="group relative bg-black rounded-2xl p-6 shadow-2xl hover:shadow-3xl transition-all duration-500 hover:-translate-y-2 animate-slide-up border border-white/10 hover:border-white/20 backdrop-blur-sm hover:bg-black/95">
      ...
      <div className={`relative inline-flex items-center gap-1.5 ${statusClass} border px-3 py-1 rounded-full text-xs font-medium mb-4 backdrop-blur-sm`}>
      ...
      <div className="relative space-y-2 mb-4 bg-white/5 rounded-xl p-3 border border-white/5">
        ...
        <div className="flex items-center justify-between">
          <span className="text-gray-400 text-xs uppercase tracking-wider">Budget</span>
      ...
      <div className="relative flex items-center justify-between text-xs border-t border-white/5 pt-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <span className="text-gray-400 block text-[10px] uppercase tracking-wider">Start</span>
            <span className="text-white text-xs">{format(new Date(project.startDate), "MMM dd, yyyy")}</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-gray-500">⏰</span>
          <div className="text-right">
            <span className="text-gray-400 block text-[10px] uppercase tracking-wider">Deadline</span>
            <span className="text-white text-xs">{format(new Date(project.deadline), "MMM dd, yyyy")}</span>
          </div>
        </div>
      </div>
      ...
      <div className="relative flex gap-2 mt-4 pt-3 border-t border-white/5">
        <button className="flex-1 bg-white/5 text-white text-sm px-3 py-2 rounded-lg hover:bg-white/10 transition-all duration-300 border border-white/5 hover:border-blue-500/30 group/btn">
        ...
        <button className="flex-1 bg-white/5 text-white text-sm px-3 py-2 rounded-lg hover:bg-white/10 transition-all duration-300 border border-white/5 hover:border-red-500/30 group/btn">
          <span className="flex items-center justify-center gap-1">
            <span>🗑️</span>
            <span className="group-hover/btn:text-red-400 transition-colors">Delete</span>
          </span>
        </button>
      </div>
    </div>
